...
// market.js'dagi mahsulotlarda "basePrice" maydoni yo'q,
// shuning uchun narxni "price" satridan (masalan "450 000 so'm") hisoblab olamiz.
function getBasePrice(product) {
    return parsePriceToNumber(product.price);
}
...
function computePricing(product, shippingMode) {
    ...
    const base = Math.round(basePrice * 1.0); // manba narxi
    const shipping = Math.round(basePrice * shippingRate);
    const fee = Math.round(basePrice * feeRate);
...
function renderPlatformTable(product) {
    ...
    const rows = PLATFORM_INFO.map((info) => {
        const rawPrice = basePrice * info.priceMult;
        const shipping = basePrice * 0.075;
...
function renderProduct(product) {
    ...
    // market.js'da "rating" maydoni yo'q — getProductRating() orqali barqaror reyting hosil qilamiz
    const rating = parseFloat(getProductRating(product));
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // "catalogProducts" o'rniga market.js'dagi haqiqiy massiv nomi: "products"
